...
LRESULT CALLBACK
tn5250_windows_printer_wndproc(
		HWND
hwnd,
UINT nMsg,
		WPARAM
...
wndclass.
lpszClassName = LP5250D_WINDOW_CLASS;
wndclass.
cbSize = sizeof(WNDCLASSEX);
wndclass.
style = CS_HREDRAW | CS_VREDRAW;
wndclass.
lpfnWndProc = tn5250_windows_printer_wndproc;
wndclass.
cbClsExtra = 0;
wndclass.
cbWndExtra = 0;
...
hInstance = i;
wndclass.
hIcon = LoadIcon(i, MAKEINTRESOURCE(IDI_TN5250_ICON));
wndclass.
hIconSm = NULL;
wndclass.
...
&rec);
MoveWindow(hWnd, rec
.left, rec.top, newwidth, newheight, FALSE);
...
nid.
uCallbackMessage = WM_TN5250_NOTIFY_ICON;
nid.
hIcon = LoadIcon(i, MAKEINTRESOURCE(IDI_TN5250_ICON));
strcpy(nid
.szTip, "5250 printer");
...
if (
tn5250_config_load_default (config)
== -1) {
...
if (
tn5250_config_parse_argv (config, argc, argv
) == -1) {
tn5250_config_unref(config);
...
tn5250_config_get (config,
"trace")) {
tn5250_log_open (tn5250_config_get(config,
...
&rc);
rc.
top = rc.bottom - buttonh;
rc.
left = ((rc.right - rc.left) - buttonw) / 2;
...
rc.
left = ((rc.right - rc.left) - buttonw) / 2;
MoveWindow(hwndStopButton, rc
.left, rc.top, buttonw, buttonh, TRUE);
break;
