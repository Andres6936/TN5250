...
BOOL CALLBACK
ConnectDlgProc (HWND, UINT, WPARAM, LPARAM
);
...
sign_key_hack = 0;
if (
tn5250_display_config (display, config
) == -1)
...
if (
tn5250_terminal_config (term, config
) == -1)
...
term->
conn_fd = tn5250_stream_socket_handle(stream);
tn5250_session_set_stream(sess, stream
);

if (
tn5250_session_config (sess, config
) == -1)
...
SendMessage (hwndMap, EM_LIMITTEXT, (WPARAM)
9, 0);
SendMessage (hwndDevice, EM_LIMITTEXT, (WPARAM)
MAX_DEVICE_SIZE, 0);
SendMessage (hwndHost, EM_LIMITTEXT, (WPARAM)
MAX_HOST_SIZE, 0);
