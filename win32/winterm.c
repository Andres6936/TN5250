...
void
win32_terminal_draw_text(HDC hdc, int attr, const char* text, int len, int x, int y, int* spacing,
		Tn5250Win32Attribute* map, int ox, int oy);

LRESULT CALLBACK
win32_terminal_wndproc( HWND
hwnd,
UINT msg, WPARAM
wParam,
LPARAM lParam
...
static void win32_terminal_init(Tn5250Terminal* This)
{
	...
	This->data->hwndMain = CreateWindow(TN5250_WNDCLASS,
			title,
			This->data->resize_fonts ? WS_OVERLAPPEDWINDOW : (WS_OVERLAPPEDWINDOW &
															  (~WS_MAXIMIZEBOX)),
			CW_USEDEFAULT,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
		...
		const char* myfont80, const char* myfont132)
{
	...
	win32_terminal_font(This, This->data->font_80, 80, 25, This->data->font_80_w,
			This->data->font_80_h);
...
/* calculate the actual size of the font we selected */
	...
	TN5250_LOG(
			("WIN32: Using font size:  Loaded with height=%d, width=%d, got height=%d, width=%d\n",
...
void win32_terminal_choosefont(HWND hwnd)
{
	...
	cf.Flags = CF_INITTOLOGFONTSTRUCT | CF_FIXEDPITCHONLY | CF_FORCEFONTEXIST | CF_LIMITSIZE |
			   CF_SCREENFONTS;
	...
	if (ChooseFont(&cf))
	{
		...
		if (tn5250_display_width (globDisplay) < 100)
		{
			...
			win32_terminal_font(globTerm, globTerm->data->font_in_use.lfFaceName,
					tn5250_display_width (globDisplay), tn5250_display_height (globDisplay) + 1,
					globTerm->data->font_80_w, globTerm->data->font_80_h);
		...
		else
		{
			...
			win32_terminal_font(globTerm, globTerm->data->font_in_use.lfFaceName,
					tn5250_display_width (globDisplay), tn5250_display_height (globDisplay) + 1,
					globTerm->data->font_132_w, globTerm->data->font_132_h);
...
globTerm->data->
selecting = 0;
globTerm->data->selstr.
x = 2;
globTerm->data->selstr.
y = 2;
globTerm->data->selend.
x =
...
tn5250_display_kf_macro(globDisplay,
K_MEMO);
if (
tn5250_macro_rstate(globDisplay)
) {
...
if (h>0 && w>0) {
int c, r;
win32_terminal_clear_screenbuf(hwnd, w, h,
1, 1);
if (globTerm!=NULL && globDisplay!=NULL) {
if (globTerm->data->resize_fonts) {
win32_calc_default_font_size(hwnd,
80, 24, &c, &r);
globTerm->data->
font_80_h = r;
globTerm->data->
...
if (
BitBlt(hdc, x, y, w, h, bmphdc, x, y, SRCCOPY
)==0) {
