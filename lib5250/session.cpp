...
tn5250_session_send_fields(Tn5250Session* This, int aidcode)
{
	...
	default:            /* Huh? */
		TN5250_LOG (
				("BUG!!! Trying to transmit fields when This->read_opcode = 0x%02X.\n", This->read_opcode));
		TN5250_ASSERT (0);
	}
...
tn5250_session_define_selection_field(Tn5250Session* This, int length)
{
   ...
   */
	...
	{
		TN5250_LOG (
				("Selection field is auto-enter on selection except if double-digit numeric selection is used\n"));
	}
	else if ((flagbyte1 & 0x0C) == 2)
	{
		TN5250_LOG (
				("Selection field is auto-enter on selection or deselection except if double-digit numeric selection is used\n"));
	}
	else
	{
		TN5250_LOG (
		...
		int count, short createnew)
{
	...
	{
		TN5250_LOG (
				("application user desires a roll-down AID if the Cursor Up key is pressed on this choice\n"));
	}

	if (flagbyte2 & 0x20)
	{
		TN5250_LOG (
				("application user desires a roll-up AID if the Cursor Down key is pressed on this choice\n"));
	}

	if (flagbyte2 & 0x10)
	{
		TN5250_LOG (
				("application user desires a roll-left AID if the Cursor Left key is pressed on this choice\n"));
	}

	if (flagbyte2 & 0x08)
	{
		TN5250_LOG (
				("application user desires a roll-right AID if the Cursor Right key is pressed on this choice\n"));
	}
	...
	{
		TN5250_LOG (
				("use this minor structure for non-GUI NWSs that are capable of creating mnemonic underscores\n"));
	}

	if (flagbyte3 & 0x20)
	{
		TN5250_LOG (
				("use this minor structure for NWS display devices that are not capable of creating underscores\n"));
	}
